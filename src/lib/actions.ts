...
export async function listResources(): Promise<{
  ...
  try {
    ...
    devError("List resources error:", err);
    const errorMessage =
...
export async function uploadResourceDocument(
  ...
  try {
    ...
    devError("Upload resource document error:", err);
    const errorMessage =
...
export async function downloadResourceFile(
  ...
  try {
    const response = await fetch(
      `/api/resources/download?bucket=${encodeURIComponent(bucket)}&path=${encodeURIComponent(path)}`,
      {
        method: "GET",
      }
    );
    ...
    devError("Download resource file error:", err);
    const errorMessage =
